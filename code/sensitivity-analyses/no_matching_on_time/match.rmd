...
treated candidates and finds the nearest reference candidate. 
If the difference in the log odds is less
...
Manually set follow-up time for everyone to 1. This has the same effect as 
ignoring it completely.

```{r def-ps-match}
ps_match <- function(treatment, control, two_rx, seed) {
  if (two_rx) {
    log_path <- glue::glue("/Shared/lss_jsimmeri/LBD/{treatment}_{control}_2rx_ps_only.txt")
  } else {
    log_path <- glue::glue("/Shared/lss_jsimmeri/LBD/{treatment}_{control}_any_ps_only.txt")
  }  
  ...
  if (two_rx) {
    data_path <- glue::glue("/Shared/lss_jsimmeri/LBD/model_{treatment}_{control}_2rx.rds")
  } else {
    data_path <- glue::glue("/Shared/lss_jsimmeri/LBD/model_{treatment}_{control}_any.rds")
  }
  ...
  if (two_rx) {
    match_path <- glue::glue("/Shared/lss_jsimmeri/LBD/matches_{treatment}_{control}_2rx_ps_only.rds")
  } else {
    match_path <- glue::glue("/Shared/lss_jsimmeri/LBD/matches_{treatment}_{control}_any_ps_only.rds")
  }
...
par_match <- function(args) {
  ...
  two_rx <- args[[3]]
  seed <- args[[4]]
  ps_match(treatment, control, two_rx, seed)
  return(0)
}
```
...
args_list <- list(
  list("tz", "tam", TRUE, seeds[1]),
  list("tz", "5ari", TRUE, seeds[2]),
  list("tam", "5ari", TRUE, seeds[3]),
  list("tz", "tam", FALSE, seeds[4]),
  list("tz", "5ari", FALSE, seeds[5]),
  list("tam", "5ari", FALSE, seeds[6])
)
```
