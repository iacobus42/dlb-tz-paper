---
title: "Find First LBD Date for All Truven Enrollees"
description: |
  Find the first date with a diagnosis of LBD or a dispensing of levodopa across
...
1. A diagnosis of LBD with ICD-9-CM 33182 or ICD-10-CM G3183

To do this extraction, we build a function to find LBD diagnosis events in the
outpatient data:
...
We stack all the claims for LBD diagnosis and, for each `enrolid`,
take the first occurence as the value of `pd_date`.

```{r list-to-tibble}
first_dlb_date <- events %>%
  enframe() %>%
  filter(purrr::map_int(value, nrow) > 0) %>%
  unnest(cols = "value") %>%
  group_by(enrolid) %>%
  summarize(dlb_date = min(date))
```

Which we then save for later use.

```{r save}
write_rds(first_dlb_date,
           "/Shared/lss_jsimmeri/LBD/first_dlb_date.rds")
```
